...
conditionset
VIEWDIST = scr.dist; % cm
SCREENCM = 46; % Width in centimeters of full screen image on monitor....40 for Iiyama
...
try
    ...
    for trial = 1:nblocks*nconds  % each 'trial' ends with a setting
        ...
        save(const.prefilename,'luminance')
    ...
    disp(['Luminance of red phosphor relative to green =' num2str(mean(lum))])
    clear all
    % Done!
    return;
